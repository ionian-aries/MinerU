...
def validate_custom_config(cfg: CustomConfig) -> None:
    ...
    # --- enhance.fallback_models（可选）：字符串列表或对象列表 ---
    fallback_models = enhance.get("fallback_models")
    if fallback_models is not None:
        if not isinstance(fallback_models, list):
            raise ValueError(
                "CustomConfig validation failed: enhance.fallback_models must be a list"
            )
        for idx, item in enumerate(fallback_models):
            if isinstance(item, str):
                if not item.strip():
                    raise ValueError(
                        f"CustomConfig validation failed: enhance.fallback_models[{idx}] "
                ...
                if not isinstance(name, str) or not name.strip():
                    raise ValueError(
                        f"CustomConfig validation failed: enhance.fallback_models[{idx}].name "
                        "must be a non-empty string"
                    )
            else:
                raise ValueError(
    ...
    # --- enhance.max_output_tokens（可选）：正整数 ---
    max_output_tokens = enhance.get("max_output_tokens")
    if max_output_tokens is not None:
        try:
            ...
            raise ValueError(
                "CustomConfig validation failed: enhance.max_output_tokens "
                "must be a positive integer"
            )

    # --- enhance.chunk_max_chars（可选）：int ≥ 500，默认 3000 ---
    chunk_max_chars = enhance.get("chunk_max_chars")
    if chunk_max_chars is not None:
        try:
            cmx = int(chunk_max_chars)
            if cmx < 500:
                raise ValueError
        except (TypeError, ValueError):
            raise ValueError(
                "CustomConfig validation failed: enhance.chunk_max_chars "
                "must be an integer >= 500"
            )

    # --- enhance.chunk_min_chars（可选）：int ≥ 50，默认 200，且必须 < chunk_max_chars ---
    chunk_min_chars = enhance.get("chunk_min_chars")
    if chunk_min_chars is not None:
        try:
            cmn = int(chunk_min_chars)
            if cmn < 50:
                raise ValueError
        except (TypeError, ValueError):
            raise ValueError(
                "CustomConfig validation failed: enhance.chunk_min_chars "
                "must be an integer >= 50"
            )
        effective_max = int(chunk_max_chars) if chunk_max_chars is not None else 3000
        if cmn >= effective_max:
            raise ValueError(
                "CustomConfig validation failed: enhance.chunk_min_chars "
                f"({cmn}) must be less than chunk_max_chars ({effective_max})"
            )

    # --- enhance.image_enhance（可选）---
    image_enhance = enhance.get("image_enhance")
    if image_enhance is not None:
        if not isinstance(image_enhance, dict):
            raise ValueError(
                "CustomConfig validation failed: enhance.image_enhance must be an object"
            )
        ie_concurrency = image_enhance.get("concurrency")
        if ie_concurrency is not None:
            try:
                if int(ie_concurrency) < 1:
                    raise ValueError
            except (TypeError, ValueError):
                raise ValueError(
                    "CustomConfig validation failed: enhance.image_enhance.concurrency "
                    "must be a positive integer"
                )
        ie_max_tokens = image_enhance.get("max_tokens")
        if ie_max_tokens is not None:
            try:
                if int(ie_max_tokens) < 50:
                    raise ValueError
            except (TypeError, ValueError):
                raise ValueError(
                    "CustomConfig validation failed: enhance.image_enhance.max_tokens "
                    "must be an integer >= 50"
                )
        ie_min_side = image_enhance.get("min_short_side_px")
        if ie_min_side is not None:
            try:
                if float(ie_min_side) < 0:
                    raise ValueError
            except (TypeError, ValueError):
                raise ValueError(
                    "CustomConfig validation failed: enhance.image_enhance.min_short_side_px "
    ...
    # --- enhance.split_marker（可选）---
    split_marker = enhance.get("split_marker")
    if split_marker is not None and not isinstance(split_marker, str):
        raise ValueError("CustomConfig validation failed: enhance.split_marker must be a string")
