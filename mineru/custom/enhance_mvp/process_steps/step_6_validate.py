...
def _validate_section_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    # 跳过的节直接通过（不做内容校验）
    if status in ("skipped_by_strategy", "skipped_no_content"):
        return item
...
def run_validate_stage(
    ...
    if section_results is not None:
        failed = sum(1 for s in section_results if s.get("status") == "failed")
        skipped = sum(1 for s in section_results if s.get("status") in ("skipped_by_strategy", "skipped_no_content"))
        logger.info(f"[ValidateStage] done: sections={len(section_results)} failed={failed} skipped={skipped}")
